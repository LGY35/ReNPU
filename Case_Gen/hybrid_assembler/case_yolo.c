...
for(int k=0; k<split_num; k++) {
    ...
    if(fmap_top_pad_en){
        asm volatile (
            ...
            NOC_cfg ( addr=7 , wdata=80, cfifo_wdata=0,cfifo_en=0 )    # 比上一个地址+80,跳到第二行(一行80个数据)  取另一半数据的时候用120 ==============
            ...
            NOC_cfg ( addr=17 , wdata=1279, cfifo_wdata=0,cfifo_en=0)  # ping length   32*40=1280
            ...
            NOC_cfg ( addr=31 , wdata=16, cfifo_wdata=0,cfifo_en=0)    # loop num 取多少行 32/2 = 16==============
        ...
        // 其他core
            ...
            // core 10 和 11 使用group addr取fmap,所以需要广播
            if(core_id == 10 || core_id == 11){
                if(k = 0){
                    asm volatile(
                        next_fetch_is_npu
                        NOC_cfg ( addr=0 , wdata=0, cfifo_wdata=0,cfifo_en=0 )     # 0 绝对寻址
