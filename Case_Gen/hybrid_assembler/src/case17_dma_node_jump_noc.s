...
NOC_cfg (addr=114 , wdata=39)   # ping lenth
NOC_cfg (addr=115 , wdata=0 )
NOC_cfg (addr=116 , wdata=0 )   
NOC_cfg (addr=117 , wdata=0 )   
NOC_cfg (addr=118 , wdata=0 )
NOC_cfg (addr=119 , wdata=1 )   # dmaloop enable
NOC_cfg (addr=116 , wdata=160 )   # dmaloop gap # gap2整行，也就是160
NOC_cfg (addr=117 , wdata=10 )   # dmaloop num    20/2   
NOC_cfg (addr=118 , wdata=80 )  # dmaloop Baddr
...
NOC_cfg (addr=99  , wdata=0 )
NOC_cfg (addr=100 , wdata=0 )
NOC_cfg (addr=101 , wdata=800 )   # ram base addr       # 取另一半的数据
NOC_cfg (addr=102 , wdata=0 )   # ram base addr
...
NOC_cfg (addr=116 , wdata=160 )   # dmaloop gap # gap2整行，也就是160
NOC_cfg (addr=117 , wdata=10 )   # dmaloop num    20/2   
NOC_cfg (addr=118 , wdata=120 )  # dmaloop Baddr
noc_req (comd_type=3, bar=0 )
noc_req (comd_type=4, bar=0 )
MQ_NOP (bar=0, nop_cycle_num=1)
MQ_NOP (bar=0, nop_cycle_num=1)
