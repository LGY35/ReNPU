hybrid_assembler_v3.9

使用说明：
1. hybrid_assembler_v3.9: cpu和npu混合汇编器，可通过-h查看usage。
2. inst_set_v3.9.s: 默认的输入文件，存放汇编指令。

功能更新：
1. 修改后的指令的同步更新
  - CU指令[VQ_CALU_SCACHE_WR_EN、VQ_CALU_SCACHE_RD_EN、conv3d_start、psum_rd、VQ_NOP、CVEC_CFG2]
  - MU指令[npu_load、npu_mv、npu_store、MQ_NOP]
  - CUB指令[cub.event_finish、cub.nop cycle_num、cub.cflow_nop cycle_num、cub.lci imm]


附录：
指令格式参考示例：
npu_load (we=wr, l1b_mode=norm, tcache_bank_num=0, sys_gap=1, sub_gap=0, sub_len=16, addr=1, sys_len=8, mv_last_dis=0, cfifo_en=1, bar=0)
...
npu_store (we=rd, l1b_mode=norm, tcache_bank_num=0, sys_gap=1, sub_gap=1, sub_len=1, addr=0, sys_len=1, mv_last_dis=0, cfifo_en=1, bar=0)
...
MQ_cfg1 (sub_gap=0, sys_gap_ext=0b10001)
MQ_cfg2 (state_clr=0)
MQ_NOP (bar=0, nop_cycle_num=1)
CVEC_cfg0 (kernel_size=3, fmap_bank_num=7, stride=0)
...
conv3d_start (first_sub_flag=0, start_index=0, end_index=29, bc_mode=0, bc_len=16, rgba_mode=0, rgba_stride=0, rgba_shift=0, hl_op=0, bc_keep_2cycle_en=0, pad0_sel=end, pad0_len=0, psum_rgb_sel=1, run_cycle_num=32, cfifo_en=1, bar=1)
...
psum_rd (rd_num=0, rd_ch_sel=0, scache_wr_addr=0, scache_wr_size=word, run_cycle_num=0, cfifo_en=0, bar=1)
...
cubank_mask (mask_sel=1, cubank_mask=1)
next_fetch_is_cpu
next_fetch_is_npu
storec x8, MQ 
storec x11, VQ
...
cub_alu_mask(mask_sel=1, cub_alu_mask=1)
cub.lp.starti 1, 1
cub.lp.endi 1, 1
cub.lp.counti 1, 1
cub.lp.setupi 1, 1, 1
